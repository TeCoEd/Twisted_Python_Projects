...
def start():
    ...
    button2.bg="Green"
    final_time.repeat(10, timer) # Edit speed
...
def stop():
    ...
    reaction_speed  = float(final_time.value) / 75
    reaction_speed = "{:.5f}".format(reaction_speed)
    app.info("Your reaction time ", "Your reaction time was " + str(reaction_speed) + " seconds")

    if float(reaction_speed) < float(old_fastest_time):
        fastest_time.value = ("FASTEST TIME = " + str(reaction_speed))
    else:
        pass

    old_high_reaction = reaction_speed
...
app = App("Reaction Timer", bg="white", height=525)
...
button1 = PushButton(app, command=start, width=20, height=1, text='Start') 
...
fastest_time = Text(app, text =" FASTEST TIME =  ")
